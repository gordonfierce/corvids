...
def multiprocessGetSolutionSpace(
    min_score,
    max_score,
    num_samples,
    mean_and_variance,
...
class RecreateData:
    ...
    def adjust_sol(self, solution, num_adjustments):
        ...
        while num_adjusted < num_adjustments:
            ...
            if correction == 1:
                candidates = [
                    item
                    for item, count in list(collections.Counter(new_sol).items())
                    if count > 1
                    and (item + 1) in self.poss_vals
                    and (item - 1) in self.poss_vals
                ]
                if len(candidates) == 0:
            ...
            for val_1, val_2 in itertools.combinations(items, 2):
                ...
                for size in range(
                    1, min((max(self.poss_vals) - val_2), val_1 - min(self.poss_vals))
                ):
                    if (
                        val_1 - size not in self.poss_vals
                        or val_2 + size not in self.poss_vals
                    ):
                        continue
            ...
            potential_diff = potential_diffs[
                -1
            ]  # Greed algorithm here to choose the biggest adjustment possible
    ...
    def validMeansVariances(self, findFirst):
        ...
        means_list = []
        for i in range(
            int(math.ceil((self.mean - self.mean_precision) * self.num_samples)),
        ...
        for m in means_list:
            ...
            initial_var = (
                (self.num_samples - m * self.num_samples % self.num_samples)
                * (math.floor(m) - m) ** 2
                + (m * self.num_samples % self.num_samples) * (math.ceil(m) - m) ** 2
            ) / (self.num_samples - 1)
            initial_mean_valid = [int(math.floor(m))] * (
                int((self.num_samples - m * self.num_samples % self.num_samples))
            ) + [int(math.ceil(m))] * int((m * self.num_samples % self.num_samples))
            ...
            initial_adjusted_var = (
                initial_var * (self.num_samples - 1) * self.num_samples**2
            )
            # print "Initial Adjusted: " + str(initial_adjusted_var)
            i = int(
                math.ceil(
                    (self.variance - self.variance_precision)
                    * ((self.num_samples - 1) * self.num_samples**2)
                )
            )
            while (
                i
                ...
                mean_variances.append(
                    (m, float(i) / ((self.num_samples - 1) * self.num_samples**2))
                )
                solution = self.adjust_sol(
                    initial_mean_valid, int((i - initial_adjusted_var) / step_size)
                )
                if solution:
                    solutions[
                        (m, float(i) / ((self.num_samples - 1) * self.num_samples**2))
        ...
        if findFirst:
            if len(solutions) > 0:
                ...
                if self.debug:
                    print(
                        str(sum([len(x) for x in self.simpleData.values()]))
                        + " unique solutions found simulatenously (not neccessarily complete!!)."
                    )
                    index = 0
        ...
        if self.debug:
            print(
                "Total potential mean/variance pairs to consider: "
                + str(len(mean_variances))
            )
    ...
    def _recreateData_piece_1(
        self, check_val=None, poss_vals=None, multiprocess=True, find_first=False
    ):
    ...
    def _findAll_piece_1_multi_proc(
        self,
        solution_spaces,
        check_val=None,
        poss_vals=None,
        ...
        if self.debug:
            print(
                "Found "
                + str(len(param_tuples) + len(self.sols))
                + " potentially viable mean/variance pairs."
            )  # Get Katherine to UPDATE this!
            print(
                "Manipulating Bases and Initial Vectors for Complete Search Guarantee"
            )
        return init_bases, init_base_vecs, param_tuples

    def _findAll_piece_2_multi_proc(
        self,
        init_bases,
        init_base_vecs,
        param_tuples,
        ...
        for basis_and_init, param_tuple in zip(bases_and_inits, param_tuples):
            ...
            # print manip_basis
            single_set_sols = multiprocess_recurse_over_solution_path(
                manip_basis, manip_base_vec
            )
            temp_sols = []
            for sol in single_set_sols:
    ...
    def _findFirst_piece_1(
        ...
        if multiprocess:
            ...
            for solution_space in solution_spaces:
                ...
                init_bases.append(basis)
            pool = mp.Pool()
            bases_and_inits = pool.map(
                multiprocessGetManipBases, list(zip(init_bases, init_base_vecs))
            )
    ...
    def recreateData(
        ...
        mean_var_pairs = self._recreateData_piece_1(
            check_val=check_val,
            poss_vals=poss_vals,
        ...
        if not mean_var_pairs:
            return None
        solution_spaces = self._recreateData_piece_2(
            mean_var_pairs,
            check_val=check_val,
            poss_vals=poss_vals,
            multiprocess=multiprocess,
        ...
        if find_first:
            return self._findFirst_piece_1(
                solution_spaces,
                check_val=check_val,
                poss_vals=poss_vals,
                multiprocess=multiprocess,
            ...
            init_bases, init_base_vecs, param_tuples = self._findAll_piece_1_multi_proc(
                solution_spaces,
                check_val=check_val,
                poss_vals=poss_vals,
                multiprocess=multiprocess,
            ...
            return self._findAll_piece_2_multi_proc(
                init_bases,
                init_base_vecs,
                param_tuples,
                check_val=check_val,
    ...
    def analyzeSkew(self):
        ...
        if not self.sols:
            if self.debug:
                print(
                    "No solutions to run analysis over.  NB: recreateData() must be run before analyzeSkew()"
                )
            raise ValueError
    ...
    def graphData(self, max_samples=20):
        ...
        if not self.sols:
            if not self.simpleData:
                if self.debug:
                    print(
                        "No solutions to run analysis over.  NB: graphData() must be run before analyzeSkew()"
                    )
                raise ValueError
            sols = []
            all_sols = []
            ...
            for index1, sol in enumerate(sols):
                ...
                for val in self.poss_vals:
                    ypos.append(val - 0.5)
                    xpos.append(index1 + 0.5)  # added + .5
            ...
            for index1, sol in enumerate(sols):
                for index2, val in enumerate(sol):
                    ypos.append(self.poss_vals[index2] - 0.5)
                    xpos.append(index1 + 0.5)  # added + .5
                    dz.append(val)
...
if __name__ == "__main__":
    ...
    RD = RecreateData(
        1, 7, 10, 3, 4, mean_precision=0.5, variance_precision=0.5
    )  # (min_score, max_score, num_samples, mean, variance, mean_precision=0.0, variance_precision=0.0)
