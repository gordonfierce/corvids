...
class FileLoader(Frame):
    def __init__(self, master, funcToPassTo):
        ...
        self.bbutton = Button(
            self.master, text="Browse", command=self.browse_open_files
        )
    ...
    def browse_open_files(self):
        ...
        self.filename = askopenfilename(
            filetypes=(("RecreateData Files", "*.rd"), ("All files", "*.*")),
            initialdir="DATA/",
        )
        self.entry.delete(0, END)
...
class FileSaver(Frame):
    def __init__(self, master, funcToPassTo):
        ...
        self.cbutton.grid(row=1, column=16)
        self.bbutton = Button(
            self.master, text="Browse", command=self.browse_save_files
        )
        self.bbutton.grid(row=1, column=19)
        self.entry.grid(row=1, column=8)
...
def lambda_make_file_loader(frameEnvirons, frameName, fileLoaderName, filenameFunction):
    ...
    :return:
    """
    vars(frameEnvirons)[fileLoaderName] = FileLoader(
        vars(frameEnvirons)[frameName], filenameFunction
    )
...
def lambda_make_file_saver(frameEnvirons, frameName, fileSaverName, filenameFunction):
    ...
    :return:
    """
    vars(frameEnvirons)[fileSaverName] = FileSaver(
        vars(frameEnvirons)[frameName], filenameFunction
    )
...
class MenuItems(Frame):
    ...
    def initUI(self):
        ...
        self.master.bind(
            "<Alt-f>", lambda entry, menu=self.fileMenu: self.openMenu(menu)
        )
    ...
    def addMenu(self, menuName, text, hotKey=None):
        ...
        if hotKey:
            self.master.bind(
                hotKey, lambda entry, menu=vars(self)[menuName]: self.openMenu(menu)
            )
