...
def doc_call_wrapper(name, doc_dir):
    return lambda a=0: webbrowser.get().open(
        "file://" + doc_dir + "/corvids_readme.html#" + name, new=2
    )
...
class PossValsText(tk.Text):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        if self._min_val is not None and self._max_val is not None:
            var_current = [
                str(n) for n in range(self._min_val.get(), self._max_val.get() + 1)
            ]
    ...
    def getPossVals(self):
        current_poss = re.split("[,\s]+", self.get("1.0", "end-1c"))
        poss_vals = [
            int(math.floor(eval(poss)))
            for poss in current_poss
            if len(poss)
            and (
                poss == "0"
                or (
                    poss
                    if poss.find("..") > -1
                    else poss.lstrip("-+").rstrip("0").rstrip(".")
                ).isdigit()
            )
...
class CoreGUI(object):
    ...
    def main_4(self, solution_spaces):
        ...
        if bool(self.find_first.get()):
            ...
            init_bases, init_base_vecs, param_tuples = (
                self.rd._findAll_piece_1_multi_proc(
                    solution_spaces,
                    check_val=self.getForcedVals(),
                    poss_vals=self.poss_vals.getPossVals(),
                    multiprocess=True,
            ...
            partial_5 = wrapped_partial(
                self.main_5, init_bases, init_base_vecs, param_tuples
            )
    ...
    def main_6(self):
        ...
        if len(self.rd.sols) > 0:
            ...
            if debug:
                print(
                    str(sum([len(x) for x in self.rd.simpleData.values()]))
    ...
    def getForcedVals(self):
        ...
        forced_vals = [
            int(math.floor(eval(forced)))
            for forced in current_forced
            if len(forced)
            and (
                forced == "0"
                or (
                    forced
                    if forced.find("..") > -1
                    else forced.lstrip("-+").rstrip("0").rstrip(".")
                ).isdigit()
            )
    ...
    def InitUI(self):
        ...
        self.window.addTopLevelMenuItem(
            self.window.fileMenu,
            "Load Settings",
        ...
        self.window.addTopLevelMenuItem(
            self.window.clearMenu, "Clear Input Fields", self.clearInputFields
        )
        self.window.addTopLevelMenuItem(
            self.window.clearMenu, "Clear Output Field", self.clearOutputField
        )
        self.window.addTopLevelMenuItem(
            self.window.clearMenu, "Clear All", self.clearAll
        )
        ...
        self.debug = IntVar()
        self.debug.set(1)
        Checkbutton(self.parent, text="Show Progress", variable=self.debug).grid(
            column=description_col, row=0, sticky=W
        )
        self.find_first = IntVar()
        self.find_first.set(0)
        Checkbutton(
            self.parent, text="Stop After First Solution", variable=self.find_first
        ).grid(column=field_col, row=0, sticky=E)

        Label(self.parent, text="Minimum Value", anchor="e").grid(
            row=1, column=description_col, sticky=N + S + E + W
        )
        ...
        min_value_help = Button(
            ...
            command=doc_call_wrapper("minimum-value", self.doc_dir),
        )  # width=14, height=14)#, text="?")
        ...
        Label(self.parent, text="Maximum Value", anchor="e").grid(
            row=2, column=description_col, sticky=N + S + E + W
        )
        ...
        max_value_help = Button(
            ...
            text="?",
            font=("TkDefaultFont", 8),
            command=doc_call_wrapper("maximum-value", self.doc_dir),
        )  # width=14, height=14)#, text="?")
        ...
        Label(self.parent, text="Mean", anchor="e").grid(
            row=3, column=description_col, sticky=N + S + E + W
        )
        self.mean = StringVar()
        self.mean.set(0.0)
        ...
        mean_help = Button(
            ...
            text="?",
            font=("TkDefaultFont", 8),
            command=doc_call_wrapper("mean", self.doc_dir),
        )  # width=14, height=14)#, text="?")
        # mean_help.config(image=help_photo)
        # mean_help.image = help_photo
        mean_help.grid(row=3, column=help_col, sticky=E + W)

        Label(self.parent, text="Mean Precision", anchor="e").grid(
            row=4, column=description_col, sticky=N + S + E + W
        )
        ...
        mean_precision_help = Button(
            ...
            text="?",
            font=("TkDefaultFont", 8),
            command=doc_call_wrapper("mean-precision", self.doc_dir),
        )  # width=14, height=14)#, text="?")
        ...
        variance_help = Button(
            ...
            text="?",
            font=("TkDefaultFont", 8),
            command=doc_call_wrapper("variance", self.doc_dir),
        )  # width=14, height=14)#, text="?")
        # variance_help.config(image=help_photo)
        # variance_help.image = help_photo
        variance_help.grid(row=6, column=help_col, sticky=E + W)

        self.variance_precision_label = Label(
            self.parent, text="Variance Precision", anchor="e"
        )
        self.variance_precision_label.grid(
            row=7, column=description_col, sticky=N + S + E + W
        )
        ...
        self.variance_precision_entry.grid(
            row=7, column=field_col, sticky=E + W, padx=15
        )
        variance_precision_help = Button(
            self.parent,
            text="?",
            font=("TkDefaultFont", 8),
            command=doc_call_wrapper("variance-precision", self.doc_dir),
        )  # width=14, height=14)#, text="?")
        ...
        Label(self.parent, text="Number of Subjects", anchor="e").grid(
            row=8, column=description_col, sticky=N + S + E + W
        )
        ...
        num_subjects_help = Button(
            ...
            text="?",
            font=("TkDefaultFont", 8),
            command=doc_call_wrapper("number-of-subjects", self.doc_dir),
        )  # width=14, height=14)#, text="?")
        ...
        Label(self.parent, text="Possible Values", anchor="e").grid(
            row=9, column=description_col, sticky=N + S + E + W
        )
        ...
        Button(self.parent, text="Show/Hide", command=self._showOrHidePossVals).grid(
            row=9, column=field_col, sticky=E + W, padx=15
        )
        self.poss_vals = PossValsText(
            self.parent,
        ...
        self.poss_vals.grid(
            row=10, columnspan=3, column=description_col, sticky=N + S + E + W
        )
        self.poss_vals.grid_remove()
        ...
        poss_vals_help = Button(
            ...
            text="?",
            font=("TkDefaultFont", 8),
            command=doc_call_wrapper("possible-values", self.doc_dir),
        )  # width=14, height=16)#, text="?")
        ...
        Label(self.parent, text="Forced Values", anchor="e").grid(
            row=11, column=description_col, sticky=N + S + E + W
        )
        Button(self.parent, text="Show/Hide", command=self._showOrHideForcedVals).grid(
            row=11, column=field_col, sticky=E + W, padx=15
        )
        ...
        self.forced_vals.grid(
            row=12, columnspan=3, column=description_col, sticky=N + S + E + W
        )
        self.forced_vals.grid_remove()
        ...
        forced_vals_help = Button(
            ...
            text="?",
            font=("TkDefaultFont", 8),
            command=doc_call_wrapper("forced-values", self.doc_dir),
        )  # width=14, height=16)#, text="?")
        ...
        Label(self.parent, text="Output:").grid(
            row=13, columnspan=3, column=description_col, sticky=E + W
        )
...
if __name__ == "__main__":
    ...
    shutil.copy(
        resource_path("docs" + file_sep + "corvids_readme.html"), directory_name
    )
    ...
    root = Tk()
    root.protocol(
        "WM_DELETE_WINDOW", wrapped_partial(root_closer, root, directory_name)
    )
